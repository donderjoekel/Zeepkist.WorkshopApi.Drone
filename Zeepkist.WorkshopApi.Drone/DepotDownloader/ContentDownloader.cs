...
namespace DepotDownloader
{
    ...
    static class ContentDownloader
    {
        ...
        public static bool InitializeSteam3(string username, string password)
        {
            ...
            {
                DepotDownloader.Logger.LogCritical("Unable to get steam3 credentials");
                return false;
            }
        ...
        private static async Task DownloadWebFile(uint appId, string fileName, string url)
        {
            ...
            {
                DepotDownloader.Logger.LogCritical("Unable to create install directories!");
                return;
            }
            ...
            {
                DepotDownloader.Logger.LogInformation("Downloading {Filename}", fileName);
        ...
        public static async Task DownloadAppAsync(uint appId, List<(uint depotId, ulong manifestId)> depotManifestIds, string branch, string os, string arch, string language, bool lv, bool isUgc)
        {
            ...
            if (!AccountHasAccess(appId))
            {
                ...
                {
                    DepotDownloader.Logger.LogInformation("Obtained FreeOnDemand license for app {AppId}", appId);
        ...
        static DepotDownloadInfo GetDepotInfo(uint depotId, uint appId, ulong manifestId, string branch)
        {
            ...
            {
                DepotDownloader.Logger.LogError("Depot {DepotId} is not available from this account.", depotId);
            ...
            if (manifestId == INVALID_MANIFEST_ID)
            {
                ...
                {
                    DepotDownloader.Logger.LogWarning(
                        "Depot {DepotId} does not have branch named '{Branch}'. Trying {DefaultBranch} branch",
                        depotId,
                        branch,
                        DEFAULT_BRANCH);
                ...
                if (manifestId == INVALID_MANIFEST_ID)
                {
                    DepotDownloader.Logger.LogError("Depot {DepotId} is missing public subsection or manifest section",
                        depotId);
                    return null;
                }
            }
            ...
            {
                DepotDownloader.Logger.LogError("No valid depot key for {DepotId}, unable to download", depotId);
                return null;
            }
            ...
            {
                DepotDownloader.Logger.LogError("Unable to create install directories");
                return null;
            }
        ...
        private static async Task DownloadSteam3Async(uint appId, List<DepotDownloadInfo> depots)
        {
            ...
            DepotDownloader.Logger.LogInformation("Total downloaded: {TotalBytes} bytes ({TotalBytesUncompressed} bytes uncompressed) from {DepotCount} depots",
            ...
            uint appId, DepotDownloadInfo depot)
        {
            ...
            DepotDownloader.Logger.LogInformation("Processing depot {DepotId}", depot.id);
            ...
            if (lastManifestId == depot.manifestId && oldProtoManifest != null)
            {
                ...
                DepotDownloader.Logger.LogInformation("Already have manifest {ManifestId} for depot {DepotId}",
                    depot.manifestId,
                    depot.id);
            ...
            else
            {
                ...
                {
                    DepotDownloader.Logger.LogInformation("Downloading depot manifest...");
            ...
            DepotDownloader.Logger.LogInformation("Manifest {ManifestId} {CreationTime}",
                depot.manifestId,
        ...
        )
        {
            ...
            DepotDownloader.Logger.LogInformation("Downloading depot {DepotId}", depot.id);

            var files = depotFilesData.filteredFiles.Where(f => !f.Flags.HasFlag(EDepotFileFlag.Directory)).ToArray();
            var networkChunkQueue =
                new ConcurrentQueue<(FileStreamData fileStreamData, ProtoManifest.FileData fileData,
            ...
            await Util.InvokeAsync(
                networkChunkQueue.Select(q => new Func<Task>(async () =>
                    await Task.Run(() => DownloadSteam3AsyncDepotFileChunk(cts,
                        appId,
                        downloadCounter,
                        depotFilesData,
                        q.fileData,
            ...
            if (depotFilesData.previousManifest != null)
            {
                ...
                foreach (var existingFileName in previousFilteredFiles)
                {
                    ...
                    File.Delete(fileFinalPath);
                    DepotDownloader.Logger.LogInformation("Deleted {FinalPath}", fileFinalPath);
            ...
            DepotDownloader.Logger.LogInformation(
                "Depot {DepotId} - Downloaded {BytesCompressed} bytes ({BytesUncompressed}) bytes uncompressed",
                depot.id,
            ...
            ConcurrentQueue<(FileStreamData, ProtoManifest.FileData, ProtoManifest.ChunkData)> networkChunkQueue)
        {
            ...
            if (!fileDidExist)
            {
                DepotDownloader.Logger.LogInformation("Pre-allocating {FinalPath}", fileFinalPath);
            ...
            else
            {
                ...
                if (oldManifestFile != null)
                {
                    ...
                    if (Config.VerifyAll || !hashMatches)
                    {
                        ...
                        if (Config.VerifyAll)
                        {
                            DepotDownloader.Logger.LogInformation("Verifying {FinalPath}", fileFinalPath);
                        }
                ...
                else
                {
                    ...
                    DepotDownloader.Logger.LogInformation("Validating {FinalPath}", fileFinalPath);
                ...
                if (neededChunks.Count() == 0)
                {
                    lock (depotDownloadCounter)
                    {
                        ...
                        DepotDownloader.Logger.LogInformation("{Percentage}% {FinalPath}",
                            ((depotDownloadCounter.SizeDownloaded / (float)depotDownloadCounter.CompleteDownloadSize) *
                             100.0f).ToString("#00.00"),
                            fileFinalPath);
            ...
            ProtoManifest.ChunkData chunk)
        {
            ...
            if (remainingChunks == 0)
            {
                var fileFinalPath = Path.Combine(depot.installDir, file.FileName);
                DepotDownloader.Logger.LogInformation("{Percentage}% {FinalPath}",
                    ((sizeDownloaded / (float)depotDownloadCounter.CompleteDownloadSize) * 100.0f).ToString(
                        "#00.00"),
                    fileFinalPath);
