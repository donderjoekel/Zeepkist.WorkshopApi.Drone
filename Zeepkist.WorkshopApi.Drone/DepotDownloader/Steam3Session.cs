...
namespace DepotDownloader
{
    class Steam3Session
    {
        ...
        public Steam3Session(SteamUser.LogOnDetails details)
        {
            ...
            DepotDownloader.Logger.LogInformation("Connecting to Steam3...");
        ...
        public void RequestAppInfo(uint appId, bool bForce = false)
        {
            ...
            Action<SteamApps.PICSTokensCallback> cbMethodTokens = appTokens =>
            {
                ...
                {
                    DepotDownloader.Logger.LogWarning("Insufficient privileges to get access token for app {AppId}", appId);
                }
            ...
            Action<SteamApps.PICSProductInfoCallback> cbMethod = appInfo =>
            {
                ...
                foreach (var app_value in appInfo.Apps)
                {
                    ...
                    DepotDownloader.Logger.LogInformation("Got AppInfo for {AppId}", app.ID);
                    AppInfo[app.ID] = app;
                }
        ...
        public void RequestDepotKey(uint depotId, uint appid = 0)
        {
            ...
            {
                completed = true;
                DepotDownloader.Logger.LogInformation("Got depot key for {DepotId} result: {Result}",
                    depotKey.DepotID,
                    depotKey.Result);
        ...
        public async Task<ulong> GetDepotManifestRequestCodeAsync(uint depotId, uint appId, ulong manifestId, string branch)
        {
            ...
            DepotDownloader.Logger.LogInformation(
                "Got manifest request code for {DepotId} {ManifestId} result: {Result}",
                depotId,
                manifestId,
                requestCode);
